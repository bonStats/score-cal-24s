...
$$\color{red}{\mathbb{E}}[S(\color{blue}{K}(\cdot\vert \tilde{y}),\color{red}{\theta})]$$
...
$$\max_{f \in \mathcal{F}}\color{red}{\mathbb{E}}[S(\color{green}{f}_\sharp\color{blue}{K}(\cdot\vert \tilde{y}),\color{red}{\theta})]$$
...
-   Strictly proper scoring rule $S$ wrt $\mathcal{P}$
...
$$\boxed{K^{\star} \equiv \underset{K \in \mathcal{K}}{\arg\max}~\mathbb{E}_{\theta \sim \bar\Pi} \mathbb{E}_{\tilde{y} \sim P(\cdot ~\vert~ \theta)}\left[w(\theta, \tilde{y}) S(K(\cdot ~\vert~ \tilde{y}),\theta) \right]}$$


where $w(\theta, \tilde{y}) = \frac{\pi(\theta)}{\bar\pi(\theta)} v(\tilde{y})$ then

$$\boxed{K^{\star}(\cdot ~\vert~ \tilde{y}) = \Pi(\cdot ~\vert~ \tilde{y})}$$ almost surely.
